-- Venom.lol v2.0 — Full Integration
...
local state = {
    -- Aimbot
    aimEnabled        = false,
    aimMode           = "Camera",   -- "Camera" / "Mouse" / "Silent"
    aimKey            = "RMB",
    onePressAim       = false,
    aimPart           = "Head",
    randomAimPart     = false,
    ...
    -- Camlock (hold key, separate from aimbot toggle)
...
local function abbreviate(n)
    ...
    for k,v in abbr do
        if math.abs(n)>=v and v>sel then sel=v; res=math.round(n/v)..k end
    end
    return res
end
...
-- ══════════════════════════════════════════
--  IsReady (full check from Open Aimbot)
-- ══════════════════════════════════════════
local function IsReady(tgt)
    ...
    if state.useOffset then
        ...
        elseif state.offsetType=="Dynamic" then
            offset = (hum2 and hum2.MoveDirection or Vector3.zero)*state.dynamicOffset/10
        else
            offset = Vector3.new(0, aimP.Position.Y*state.staticOffset/10, 0)
                   + (hum2 and hum2.MoveDirection or Vector3.zero)*state.dynamicOffset/10
        end
    end
...
-- ══════════════════════════════════════════
--  SILENT AIM HANDLER (from Open Aimbot)
-- ══════════════════════════════════════════
local ValidArgs = {
...
if hasHookMeta then
    local OldIndex; OldIndex = getfenv().hookmetamethod(game,"__index",getfenv().newcclosure(function(self,idx)
        if not getfenv().checkcaller()
            and state.aimMode=="Silent"
            and Aiming
            and IsReady(Target) and select(3,IsReady(Target))[2]
            and calcChance(state.silentChance)
            and self==Mouse then
...
local function makeESPColor(plr)
    if state.rainbowVisuals then
        local h = os.clock()%state.rainbowDelay/state.rainbowDelay
        return Color3.fromHSV(h,1,1)
    end
    if state.useTeamColor and plr and plr.TeamColor then
...
local function getOrCreateESP(plr)
    ...
    d.name = newDraw("Text",{Visible=false, Color=PURPLE, Size=state.espOpacity*20, Center=true, Outline=true,
        OutlineColor=Color3.new(0,0,0), Transparency=state.espOpacity,
        Font=Drawing.Fonts and Drawing.Fonts.UI or 0})
    d.health = newDraw("Text",{Visible=false, Color=Color3.fromRGB(80,255,80), Size=12, Center=true,
    ...
    d.magnitude = newDraw("Text",{Visible=false, Color=PURPLE, Size=11, Center=true,
        Outline=true, OutlineColor=Color3.new(0,0,0), Transparency=state.espOpacity,
        Font=Drawing.Fonts and Drawing.Fonts.UI or 0})
    d.tracer = newDraw("Line",{Visible=false, Color=PURPLE, Thickness=tracerThickness, Transparency=state.espOpacity})
    espObjects[plr] = d
    return d
end
...
local function hideESP(d)
    ...
    for _,key in {"box","name","health","magnitude","tracer"} do
...
local function updateESPForPlayer(plr)
    ...
    local headSP       = Camera:WorldToViewportPoint(head.Position + Vector3.new(0, head.Size.Y/2+0.1,0))
    local bottomSP     = Camera:WorldToViewportPoint(hrp.Position - Vector3.new(0, hrp.Size.Y/2+0.3,0))
    if not vis then hideESP(d) return end

    local boxH  = math.abs(bottomSP.Y - headSP.Y)
    ...
    if d.box then
        ...
        d.box.Color     = col
        d.box.Thickness = state.espThickness
    ...
    if d.health then
        ...
        d.health.Text      = string.format("[%s%%]", abbreviate(math.floor(hum.Health)))
    ...
    if d.magnitude then
        ...
        d.magnitude.Text      = string.format("[%sm]", abbreviate(dist))
        d.magnitude.Position  = Vector2.new(hrpSP.X, hrpSP.Y)
    ...
    if state.chams and ShowESP then
        for _,p in char:GetDescendants() do
            if p:IsA("BasePart") then p.Material=Enum.Material.Neon; p.Color=PURPLE_DIM end
        end
    end
end
...
local function updateFovDraw()
    ...
    if not FovDraw then
        FovDraw = newDraw("Circle",{
            ...
            Radius=state.fovRadius, Thickness=state.fovThickness,
            Transparency=state.fovOpacity, Filled=state.fovFilled,
...
local function buildCrosshair()
    ...
    local col = PURPLE
    local defs = {
        Plus   = {{UDim2.new(0,-s,0,-1),UDim2.new(0,s*2,0,2)},
...
LogoSub.BackgroundTransparency=1; LogoSub.Text="v2.0"
...
LL.Padding=UDim.new(0,3); LL.Parent=LeftScroll
...
RL.Padding=UDim.new(0,3); RL.Parent=RightScroll
...
local function KB(name,def,cb,isR)
    ...
    local ls=false
    kBtn.MouseButton1Click:Connect(function() ls=true; kBtn.Text="[...]"; kBtn.TextColor3=TEXT end)
    UserInputService.InputBegan:Connect(function(inp)
        if not ls then return end
        if inp.UserInputType==Enum.UserInputType.Keyboard then
            ls=false; local kn=inp.KeyCode.Name
            kBtn.Text="["..kn.."]"; kBtn.TextColor3=PURPLE; if cb then cb(kn) end
        end
    end)
    return card
end
...
addR(tAimbot, TG("Camlock Enable","",state.camlockEnabled,function(on) state.camlockEnabled=on end,true))
...
addL(tBots, TG("SpinBot Enable","",state.spinBot,function(on)
    state.spinBot=on; if not on then Spinning=false end
end,false))
...
if hasMouse1Click then
    ...
    addR(tBots, TG("TriggerBot Enable","",state.triggerBot,function(on)
        state.triggerBot=on; if not on then Triggering=false end
    end,true))
...
addL(tVisuals, SLD("FOV Thickness",1,10,state.fovThickness,"",function(v)
    state.fovThickness=v
end,false))
addL(tVisuals, SLD("FOV Opacity",1,10,math.round(state.fovOpacity*10),"",function(v)
    state.fovOpacity=v/10
end,false))
...
addL(tVisuals, TG("Tracers","",state.espTracers,function(on)
    state.espTracers=on
    if not on then for _,d in espObjects do if d.tracer then pcall(function() d.tracer.Visible=false end) end end end
end,false))
...
addR(tVisuals, SLD("Thickness",1,10,state.espThickness,"",function(v)
    state.espThickness=v; tracerThickness=v
end,true))
addR(tVisuals, SLD("Opacity",1,10,math.round(state.espOpacity*10),"",function(v)
    state.espOpacity=v/10
end,true))
...
-- ══════════════════════════════════════════
--  ADDONS TAB  (fully fixed)
-- ══════════════════════════════════════════
addL(tAddons, SL("Crosshair", false))
addL(tAddons, TG("Enable","",state.crosshair,function(on)
...
addR(tAddons, SL("Notifications", true))
addR(tAddons, TG("Aim Notifications","",true,function(on) end,true))
addR(tAddons, TL("Shown in output when aimbot activates",true))

addR(tAddons, SL("Misc", true))
addR(tAddons, TG("Rainbow ESP Pulse","",false,function(on)
    state.rainbowVisuals=on
end,true))
addR(tAddons, TL("Cycles ESP color through rainbow",true))
...
addR(tAddons, TL("TriggerKey  →  hold triggerbot", true))
...
addR(tConfig, TL("Drawing API: "..(hasDrawing and "✔" or "✘"), true))
...
UserInputService.InputBegan:Connect(function(inp, gpe)
    ...
    if state.spinBot then
        ...
        if sk and (inp.KeyCode==sk or inp.UserInputType==sk) then
            if state.onePressSpinning then Spinning=not Spinning
    ...
    if state.triggerBot and hasMouse1Click then
        ...
        if tk and (inp.KeyCode==tk or inp.UserInputType==tk) then
            if state.onePressTrigger then Triggering=not Triggering
...
UserInputService.InputEnded:Connect(function(inp)
    ...
    -- Release camlock if hold mode
    if not state.camlockToggle then
        local ck=getKey(state.camlockKeybind)
        if ck and inp.KeyCode==ck then
            state.camlockEnabled=false; stickyTarget=nil
        end
    end
    -- Release spinbot/triggerbot
...
-- ══════════════════════════════════════════
--  MAIN RENDER LOOP (Aimbot + Bots + ESP)
-- ══════════════════════════════════════════
RunService.RenderStepped:Connect(function()
    ...
    if Aiming and state.aimEnabled then
        ...
        if not IsReady(Target) then
            if Target and state.offAfterKill then
                Aiming=false; Target=nil; return
            end
        ...
        if ready then
            ...
            elseif hasHookMeta and state.aimMode=="Silent" then
                -- handled by hookmetamethod above
            end
        else
            Target=nil
            if AimTween then AimTween:Cancel(); AimTween=nil end
    ...
    if state.camlockEnabled then
        ...
        stickyTarget = stickyTarget or (function()
        ...
        local tgt=stickyTarget
        if tgt and tgt.Character then
            local part=tgt.Character:FindFirstChild(state.camlockPart)
            ...
            if part then
                ...
                if state.useAdvanced and state.predictX>0 then
                    ...
                    if hrp then
                        tPos=tPos+hrp.Velocity*Vector3.new(
                            state.predictX*0.016, state.predictY*0.016, state.predictX*0.016)
                    end
                end
                local tCF=CFrame.lookAt(origin,tPos)
                if state.useAdvanced and state.smoothingOn then
                    ...
                    Camera.CFrame=Camera.CFrame:Lerp(tCF,sm)
                else
                    Camera.CFrame=tCF
                end
            end
...
RunService.Heartbeat:Connect(function()
    ...
    if hum then
        ...
        if state.speedBoost then
            hum.WalkSpeed=80
        elseif state.sprintEnabled and UserInputService:IsKeyDown(Enum.KeyCode.LeftShift) and not state.flyEnabled then
...
RunService.RenderStepped:Connect(function()
    ...
    if UserInputService:IsKeyDown(Enum.KeyCode.LeftShift) and not state.flyEnabled then
        -- don't use shift for fly-down if sprint is on
...
Player.CharacterAdded:Connect(function()
    state.flyEnabled=false; bv=nil; bg=nil; stickyTarget=nil; Target=nil; Aiming=false
end)
...
print("[Venom.lol v2.0] Loaded ✓")
print("  Drawing: "..(hasDrawing and "✔" or "✘"))
